...
async def test_baseline_mode_cum_visible_equals_cum_baseline(banks_dir):
    ...
    try:
        page = FakePageDriver(
            pages, start_url="https://news.ycombinator.com/",
        ...
        runner = AgentRunner(
            ...
            session_id="baseline-test", mode="baseline",
...
async def test_baseline_mode_never_injects_banks(banks_dir):
    ...
    try:
        ...
        counter = get_token_counter(prefer_hf=False)
        config = RunnerConfig(max_steps=2, dom_token_cap=4000,
                              stream_frames=False, log_clickhouse=False)
        runner = AgentRunner(
            page=page, client=client, counter=counter, config=config,
            task="Go", session_id="baseline-no-bank", mode="baseline",
            step_logger=StepLogger(None), metrics=Metrics(),
        )
